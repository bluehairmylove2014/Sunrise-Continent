import { GUEST_RATINGS } from "./Rating.constant";

export const FILTER_SEARCH = {
  NAME: "filter board",
};
export const LOCATION_TYPES = {
  HOTEL: {
    INPUT_NAME: "hotel",
    LABEL: "Khách sạn",
  },
  RESORT: {
    INPUT_NAME: "resort",
    LABEL: "Resort",
  },
  INN: {
    INPUT_NAME: "inn",
    LABEL: "Nhà nghỉ",
  },
  FULL_APARTMENT: {
    INPUT_NAME: "full_apartment",
    LABEL: "Toàn bộ căn hộ",
  },
  SERVICE_APARTMENT: {
    INPUT_NAME: "service_apartment",
    LABEL: "Căn hộ dịch vụ",
  },
  HOMESTAY: {
    INPUT_NAME: "homestay",
    LABEL: "Homestay",
  },
  ENTIRE_HOUSE: {
    INPUT_NAME: "entire_house",
    LABEL: "Toàn bộ nhà riêng",
  },
  MOTEL: {
    INPUT_NAME: "motel",
    LABEL: "Khách sạn con nhộng",
  },
  LOVE_HOTEL: {
    INPUT_NAME: "love_hotel",
    LABEL: "Khách sạn tình yêu",
  },
  PRIVATE_VILLA: {
    INPUT_NAME: "private_villa",
    LABEL: "Biệt thự tư nhân",
  },
  LODGE: {
    INPUT_NAME: "lodge",
    LABEL: "Lodge",
  },
  SUBURBAN_HOUSE: {
    INPUT_NAME: "suburban_house",
    LABEL: "Nhà ở ngoại ô",
  },
  TENT: {
    INPUT_NAME: "tent",
    LABEL: "Lều",
  },
  CHALET: {
    INPUT_NAME: "chalet",
    LABEL: "Chalet",
  },
  FARM: {
    INPUT_NAME: "farm",
    LABEL: "Nông trại",
  },
  BOAT: {
    INPUT_NAME: "boat",
    LABEL: "Thuyền",
  },
};
export const BED_TYPES = {
  DOUBLE: {
    INPUT_NAME: "double",
    LABEL: "Giường đôi",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/bed_types/double-bed.png",
  },
  SINGLE_OR_TWIN: {
    INPUT_NAME: "single_or_twin",
    LABEL: "Giường đơn / hai giường đơn",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/bed_types/bed.png",
  },
  LARGE: {
    INPUT_NAME: "large",
    LABEL: "Giường lớn",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/bed_types/single-bed.png",
  },
  LARGE_DOUBLE: {
    INPUT_NAME: "large_double",
    LABEL: "Giường đôi lớn",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/bed_types/double-bed (2).png",
  },
  BUNK: {
    INPUT_NAME: "bunk",
    LABEL: "Giường tầng",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/bed_types/bunk-bed.png",
  },
};
export const ROOM_OPTIONS = {
  BREAKFAST_INCLUDED: {
    INPUT_NAME: "breakfast_included",
    LABEL: "Có ăn sáng",
    ICON: "fi fi-ss-soup",
  },
  DINNER_INCLUDED: {
    INPUT_NAME: "dinner_included",
    LABEL: "Bao gồm bữa tối",
    ICON: "fi fi-sr-room-service",
  },
  FREE_SNACKS: {
    INPUT_NAME: "free_snacks",
    LABEL: "Miễn phí đồ ăn nhẹ",
    ICON: "fi fi-sr-hamburger-soda",
  },
  LUNCH_INCLUDED: {
    INPUT_NAME: "lunch_included",
    LABEL: "Bao gồm bữa trưa",
    ICON: "fi fi-sr-burger-fries",
  },
  AFTERNOON_TEA: {
    INPUT_NAME: "afternoon_tea",
    LABEL: "Trà chiều",
    ICON: "fi fi-ss-mug-hot-alt",
  },
};
export const ACCOMMODATION_FACILITIES = {
  SWIMMING_POOL: {
    INPUT_NAME: "swimming_pool",
    LABEL: "Bể bơi",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/swimming-pool.png?w=50&h=50",
  },
  INTERNET: {
    INPUT_NAME: "internet",
    LABEL: "Internet",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/wifi.png?w=50&h=50",
  },
  PARKING: {
    INPUT_NAME: "parking",
    LABEL: "Bãi để xe",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/parking-car.png?w=50&h=50",
  },
  AIRPORT_SHUTTLE: {
    INPUT_NAME: "airport_shuttle",
    LABEL: "Đưa đón sân bay",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/shuttle.png?w=50&h=50",
  },
  GYM: {
    INPUT_NAME: "gym",
    LABEL: "Phòng tập",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/weightlifter.png?w=50&h=50",
  },
  RECEPTION_24H: {
    INPUT_NAME: "reception_24h",
    LABEL: "Bàn tiếp tân [24 giờ]",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/reception.png?w=50&h=50",
  },
  FAMILY_FRIENDLY: {
    INPUT_NAME: "family_friendly",
    LABEL: "Thích hợp cho gia đình/trẻ em",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/family.png?w=50&h=50",
  },
  NON_SMOKING: {
    INPUT_NAME: "non_smoking",
    LABEL: "Không hút thuốc",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/non-smoking-room.png?w=50&h=50",
  },
  SPA_SAUNA: {
    INPUT_NAME: "spa_sauna",
    LABEL: "Spa/xông khô",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/sauna.png?w=50&h=50",
  },
  RESTAURANT: {
    INPUT_NAME: "restaurant",
    LABEL: "Nhà hàng",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/restaurant.png?w=50&h=50",
  },
  SMOKING_AREA: {
    INPUT_NAME: "smoking_area",
    LABEL: "Khu vực hút thuốc",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/smoking-area.png?w=50&h=50",
  },
  PETS_ALLOWED: {
    INPUT_NAME: "pets_allowed",
    LABEL: "Được phép đưa thú nuôi vào",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/dog.png?w=50&h=50",
  },
  NIGHT_CLUB: {
    INPUT_NAME: "night_club",
    LABEL: "Câu lạc bộ đêm",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/disco-ball.png?w=50&h=50",
  },
  DISABLED_FACILITIES: {
    INPUT_NAME: "disabled_facilities",
    LABEL: "Tiện nghi cho khách khuyết tật",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/disability.png?w=50&h=50",
  },
  WORK_FACILITIES: {
    INPUT_NAME: "work_facilities",
    LABEL: "Tiện nghi làm việc",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/hard-work.png?w=50&h=50",
  },
  GOLF_COURSE: {
    INPUT_NAME: "golf_course",
    LABEL: "Sân gôn [tại chỗ nghỉ]",
    ICON: "https://rialloer.sirv.com/Sunrise-Continent/ACCOMMODATION_FACILITIES/golf-field.png?w=50&h=50",
  },
};

export const FILTER_CHECKBOX_KEY = [
  {
    checkboxGroupKey: "hotelType",
    label: "Loại hình nơi ở",
    checkboxData: LOCATION_TYPES,
  },
  {
    checkboxGroupKey: "bedType",
    label: "Loại giường",
    checkboxData: BED_TYPES,
  },
  {
    checkboxGroupKey: "guestRating",
    label: "Đánh giá của khách",
    checkboxData: GUEST_RATINGS,
  },
  {
    checkboxGroupKey: "facilities",
    label: "Tiện nghi chỗ nghỉ",
    checkboxData: ACCOMMODATION_FACILITIES,
  },
  {
    checkboxGroupKey: "amenities",
    label: "Chọn phòng có",
    checkboxData: ROOM_OPTIONS,
  },
];
