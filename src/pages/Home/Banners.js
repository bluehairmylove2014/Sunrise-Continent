...
const Banners = ({ data_banner }) => {
  ...
  const handlePrev = () => {
    setActiveIndex(
      (prevIndex) => (prevIndex - 1 + data_banner.length) % data_banner.length
    );
  };
  ...
  return (
    ...
    <div className="home-banner__slider" {...swipeHandlers}>
      <div className="home-banner__slides">{banners}</div>
      <button className="home-banner__prev" onClick={handlePrev}>
        Prev
      </button>
      <button className="home-banner__next" onClick={handleNext}>
        Next
      </button>
      <div className="home-banner__indicators">{indicators}</div>
    </div>
  );
};
